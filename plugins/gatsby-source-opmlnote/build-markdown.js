...
function buildMarkdown(oPost, timezone) {
  const created = dayjs.utc(
    oPost.created,
    'ddd, D MMM YYYY HH:mm:ss [GMT]',
    true
  );
  const type = oPost.type || 'outline';
...
function buildSection(oOutline, level, type) {
  ...
  switch (type) {
    case 'ul':
      if (oOutline.inList) {
        prefix = ' '.repeat(
          oOutline.text.length - oOutline.text.trimLeft().length
        );
      } else {
        prefix = '';
      }
      ...
      for (const oChild of oOutline.outline) {
        oChild.text = `${
          prefix + (oOutline.inList ? '    ' : '')
        }* ${oChild.text.trim().replace(/^[* ]+/, '')}`;
        oChild.inList = true;
        if (oChild.outline) {
    ...
    case 'ol':
      ...
      for (const oChild of oOutline.outline) {
        oChild.text = `${
          prefix + (oOutline.inList ? '    ' : '') + ulinc++
        }. ${oChild.text.trim().replace(/^[\d]+\. +/, '')}`;
        oChild.inList = true;
        if (oChild.outline) {
          body += buildSection(oChild, level + 1, type);
        } else {
